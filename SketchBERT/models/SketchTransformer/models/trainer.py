...
from .networks import SketchTransformerModel, MaskSketchRecModel, MaskSketchGMMModel, SketchClassificationModel, \
    SketchClsPoolingModel, SketchRetrievalPoolingModel, SketchDiscretePoolingModel, GMMLoss
from .utils import strokes2drawing, to_normal_strokes, rec_incomplete_strokes, disc2stroke5

from torch.distributions.multivariate_normal import MultivariateNormal
from torch.distributions.one_hot_categorical import OneHotCategorical

TASKMODELS = {'maskrec': MaskSketchRecModel, 'maskgmm': MaskSketchGMMModel, 'maskdisc': SketchDiscretePoolingModel,
              'sketchcls': SketchClassificationModel, 'sketchclsinput': SketchClsPoolingModel,
              'sketchretrieval': SketchRetrievalPoolingModel}
...
class SketchTransformerTrainer(Trainer):
    ...
    def __init__(self, args, datas, log_dir, logger, tensorboard_logger, **kwargs):
        ...
        self.running_paras = {'output_attentions': self.args.output_attentions,
                              'output_all_states': self.args.output_all_states,
                              'keep_multihead_output': self.args.keep_multihead_output}
    ...
    def build_models(self, args):
        ...
        # Encoder
        netE_kwargs = {
            'model_type': args.model_type, 'layers_setting': args.layers_setting,
            'embed_layers_setting': args.embed_layers_setting, 'input_dim': args.input_dim,
            'max_length': args.max_length + self.cls_in_input + self.rel_in_input, 'max_size': args.max_size,
            'type_size': args.type_size,
            'position_type': args.position_type, 'segment_type': args.segment_type,
            'sketch_embed_type': args.sketch_embed_type, 'embed_pool_type': args.embed_pool_type,
            'attention_norm_type': args.attention_norm_type, 'inter_activation': args.inter_activation,
        ...
        TASKPARAS = {'maskrec': {'rec_layers_setting': args.rec_layers_setting, 'hidden_dim': args.layers_setting[0][1],
                                 'input_dim': args.input_dim, 'cls_in_input': self.cls_in_input,
                                 'rel_in_input': self.rel_in_input},
                     'maskgmm': {'hidden_dim': args.layers_setting[0][1], 'M': args.M,
                                 'cls_in_input': self.cls_in_input, 'rel_in_input': self.rel_in_input},
                     'maskdisc': {'hidden_dim': args.layers_setting[0][1], 'max_size': args.max_size,
                                  'type_size': args.type_size, 'cls_in_input': self.cls_in_input,
                                  'rel_in_input': self.rel_in_input},
                     'sketchclsinput': {'cls_layers_setting': args.cls_layers_setting,
                                        'hidden_dim': args.layers_setting[0][1], 'cls_dim': args.cls_dim,
                                        'pool_dim': self.rel_in_input},
                     'sketchretrieval': {'rel_layers_setting': args.rel_layers_setting,
                                         'hidden_dim': args.layers_setting[0][1], 'feat_dim': args.rel_feat_dim,
                                         'pool_dim': 0}}
        ...
        self.losses = {'mask_axis': F.l1_loss, 'mask_disc': nn.CrossEntropyLoss(), 'mask_type': nn.CrossEntropyLoss(),
                       'prediction': nn.CrossEntropyLoss(), 'gmm': GMMLoss(),
        ...
        self.losses_weights = {'mask_axis': args.mask_axis_weight, 'rec_axis': args.rec_axis_weight,
                               ...
                               'prediction': args.prediction_weight, 'triplet': args.triplet_weight}
    ...
    def restore_models(self):
        ...
        if 'enc_net' in self.args.which_pretrained:
            netE.load_model(checkpoint['netE_state'], self.rel_in_input, self.cls_in_input,
                            'sketchretrieval' in checkpoint['args']['_cfg_dict']['task_types'],
                            'sketchclsinput' in checkpoint['args']['_cfg_dict']['task_types'])
    ...
    def decode_parameters(self, y, mask):
        ...
        qs = y[:, :, 6 * M:6 * M + 3]
        if mask is not None:
            return pis[mask[:, :, 0] == 0, :], mus[mask[:, :, 0] == 0, :, :], sigmas[mask[:, :, 0] == 0, :, :], rhos[
                                                                                                                mask[:,
                                                                                                                :,
                                                                                                                0] == 0,
                                                                                                                :], qs
    ...
    def recon_sketches(self, pis, mus, sigmas, rhos, qs, gamma):
        ...
        for i in range(batch_size):
            ...
            for j in range(seq_len):
                ...
                mu, sigma, rho, q = mus[i, j][comp_choice].view(-1), sigmas[i, j][comp_choice].view(-1), rhos[i, j][
                    comp_choice].view(-1), qs[i, j].view(-1)

                cov = (torch.diag((sigma * sigma)) + (1 - torch.eye(2).to(mu.device)) * rho * torch.prod(sigma)).to(
                    device=mu.device)
    ...
    def train(self):
        ...
        for epoch_i in range(self.args.num_epoch):
            ...
            for data_i, batch_data in enumerate(self.datas['train_loader']):
                ...
                batch_data = [[t.to(device=self.gpu_ids['netE'], dtype=torch.float32) for t in tensor] for tensor in
                              batch_data]
                ...
                for mask_input_state, length_mask, segment in zip(mask_input_states, length_masks, segments):
                    output_state = netE(mask_input_state, length_mask, segments=segment, head_mask=None,
                                        **self.running_paras)
                ...
                total_loss, losses = self.calculate_losses(input_states, length_masks, input_masks, pooled_outputs,
                                                           targets)
    ...
    def validate(self, output=False):
        ...
        with torch.no_grad():
            ...
            for data_i, batch_data in enumerate(self.datas['val_loader']):
                ...
                if 'maskdisc' in self.args.task_types:
                    strokes_pred = torch.cat([pooled_outputs[0]['maskdisc'][0].argmax(dim=2, keepdim=True),
                                              pooled_outputs[0]['maskdisc'][1].argmax(dim=2, keepdim=True),
                                              pooled_outputs[0]['maskdisc'][2].argmax(dim=2, keepdim=True)], dim=2).to(
        ...
        if 'sketchretrieval' in self.args.task_types:
            retrieval_evaluations = self.retrieval_evaluation(torch.cat(total_retrieval_outputs, dim=0),
                                                              torch.cat(total_targets, dim=0), self.cate_num, topk=topk)
        ...
        if total_strokes_pred[0] is not None:
            ...
            self.update_stroke_results('val', total_strokes, total_strokes_pred, total_input_masks, total_targets,
                                       output)
    ...
    def calculate_losses(self, input_states, length_masks, input_masks, pooled_outputs, targets):
        ...
        input_state, length_mask, input_mask, pooled_output, target = [input_states[0], length_masks[0], input_masks[0],
        ...
        if 'maskgmm' in self.args.task_types:
            ...
            losses['mask_gmm'] = self.losses_weights['mask_gmm'] * self.losses['gmm'](
                input_state[:, :, :2][input_mask[:, :, 0] == 0,], pis, mus, sigmas, rhos)
            pis, mus, sigmas, rhos, qs = self.decode_parameters(pooled_output['maskgmm'], 1 - input_mask)
            losses['rec_gmm'] = self.losses_weights['rec_gmm'] * self.losses['gmm'](
                input_state[:, :, :2][input_mask[:, :, 0] == 1,], pis, mus, sigmas, rhos)
            total_loss = total_loss + losses['mask_gmm'] + losses['rec_gmm']

            losses['mask_type'] = self.losses_weights['mask_type'] * self.losses['mask_type'](
                qs[input_mask[:, :, 2] == 0, :], torch.argmax(input_state[:, :, 2:5], dim=2)[input_mask[:, :, 2] == 0])
            losses['rec_type'] = self.losses_weights['rec_type'] * self.losses['mask_type'](
                qs[input_mask[:, :, 2] == 1, :], torch.argmax(input_state[:, :, 2:5], dim=2)[input_mask[:, :, 2] == 1])
        ...
        if 'maskrec' in self.args.task_types:
            ...
            losses['mask_axis'] = self.losses_weights['mask_axis'] * self.losses['mask_axis'](
                pooled_output['maskrec'][:, :, axis_dim][input_mask[:, :, axis_dim] == 0],
                input_state[:, :, axis_dim][input_mask[:, :, axis_dim] == 0])
            losses['rec_axis'] = self.losses_weights['rec_axis'] * self.losses['mask_axis'](
                pooled_output['maskrec'][:, :, axis_dim][input_mask[:, :, axis_dim] == 1],
                input_state[:, :, axis_dim][input_mask[:, :, axis_dim] == 1])
            ...
            losses['mask_type'] = self.losses_weights['mask_type'] * self.losses['mask_type'](
                pooled_output['maskrec'][:, :, 2:5][input_mask[:, :, 2] == 0, :],
                torch.argmax(input_state[:, :, 2:5], dim=2)[input_mask[:, :, 2] == 0])
            losses['rec_type'] = self.losses_weights['rec_type'] * self.losses['mask_type'](
                pooled_output['maskrec'][:, :, 2:5][input_mask[:, :, 2] == 1, :],
                torch.argmax(input_state[:, :, 2:5], dim=2)[input_mask[:, :, 2] == 1])
            total_loss = total_loss + losses['mask_type'] + losses['rec_type']

        if 'maskdisc' in self.args.task_types:
            losses['x_mask_disc'] = self.losses_weights['mask_axis'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][0][input_mask[:, :, 0] == 0, :],
                (input_state[:, :, 0][input_mask[:, :, 0] == 0]).to(torch.long))
            losses['x_rec_disc'] = self.losses_weights['rec_axis'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][0][input_mask[:, :, 0] == 1, :],
                (input_state[:, :, 0][input_mask[:, :, 0] == 1]).to(torch.long))
            losses['y_mask_disc'] = self.losses_weights['mask_axis'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][1][input_mask[:, :, 1] == 0, :],
                (input_state[:, :, 1][input_mask[:, :, 1] == 0]).to(torch.long))
            losses['y_rec_disc'] = self.losses_weights['rec_axis'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][1][input_mask[:, :, 1] == 1, :],
                (input_state[:, :, 1][input_mask[:, :, 1] == 1]).to(torch.long))
            losses['type_mask_disc'] = self.losses_weights['mask_type'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][2][input_mask[:, :, 2] == 0, :],
                (input_state[:, :, 2][input_mask[:, :, 2] == 0]).to(torch.long))
            losses['type_rec_disc'] = self.losses_weights['rec_type'] * self.losses['mask_disc'](
                pooled_output['maskdisc'][2][input_mask[:, :, 2] == 1, :],
                (input_state[:, :, 2][input_mask[:, :, 2] == 1]).to(torch.long))
            total_loss = total_loss + losses['x_mask_disc'] + losses['y_mask_disc'] + losses['type_mask_disc'] + losses[
                'x_rec_disc'] + losses['y_rec_disc'] + losses['type_rec_disc']

        if 'sketchclsinput' in self.args.task_types:
            losses['prediction'] = self.losses_weights['prediction'] * self.losses['prediction'](
                pooled_output['sketchclsinput'], target)
            total_loss = total_loss + losses['prediction']

        if 'sketchretrieval' in self.args.task_types:
            losses['triplet'] = self.losses_weights['triplet'] * self.losses['triplet'](
                *[pooled_output['sketchretrieval'] for pooled_output in pooled_outputs])
    ...
    def sketch_retrieval_evaluation(self, val_loader, topk=(1, 5), batch_size=32):
        ...
        with torch.no_grad():
            for data_i, batch_data in enumerate(val_loader):
                ...
                mask_input_states, input_states, segments, length_mask, input_mask, target = [term[0] for term in
                                                                                              batch_data]
                target = target.to(dtype=torch.long)

                output_states = netE(input_states, length_mask, segments=segments, head_mask=None,
                                     **self.running_paras)  # [batch, seq_len, hidden_dim]
    ...
    def retrieval_evaluation(self, test_states, test_targets, cate_num, collection_states=None, collection_targets=None,
                             topk=(1, 5), batch_size=32):
        ...
        for test_i in range(num_test_batch):
            ...
            for collect_i in range(num_collect_batch):
                ...
                collect_batch = collection_states[collect_i * batch_size:(collect_i + 1) * batch_size,
                                :]  # [batch, feat_dim]
    ...
    def evaluate(self, pooled_outputs, targets, topk=(1, 5)):
        ...
        if 'sketchretrieval' in self.args.task_types:
            retrieval_evaluations = self.retrieval_evaluation(pooled_outputs[0]['sketchretrieval'], targets[0],
                                                              self.cate_num, topk=(1,))
    ...
    def initialize_validate_losses(self):
        total_losses = {}
        if 'maskrec' in self.args.task_types:
            types = ['mask_axis', 'rec_axis', 'mask_type',
                     'rec_type']  # , 'mask_type', 'rec_type' 'mask_axis', 'rec_axis', 'mask_type', 'rec_type'
            for t in types:
    ...
    def display_results(self, mode, losses, evaluations):
        if mode == 'train':
            result_message = 'Train Epoch {}, Time:{:.3f}[T:{:.3f}/D:{:.3f}], Total Iter [{}/{}], [{}/{}]:'.format(
                self.counters['epoch'], self.batch_time, self.run_time, self.data_time, self.counters['t'],
                self.args.num_iterations, self.counters['now_t'], self.iter_per_epoch)
        elif mode == 'val':
            result_message = 'Validate Epoch {}, Time:{:.3f}[T:{:.3f}/D:{:.3f}], Total Iter [{}/{}], [{}/{}]:'.format(
                self.counters['epoch'], self.batch_time, self.run_time, self.data_time, self.counters['t'],
                self.args.num_iterations, self.counters['now_t'], self.iter_per_epoch)
        if losses is not None:
        ...
        if evaluations is not None:
            evaluations_message = ', '.join(
                ['[{}_{}]:{:.4f}'.format(mode, key, evaluation) for key, evaluation in evaluations.items()])
    ...
    def update_stroke_results(self, mode, strokes, strokes_pred, masks, targets, output=False, size=128):
        imgs = []
        imgs_incomp = []
        imgs_comp_pred = []
        imgs_pred = []
        ...
        for i in range(min(len(strokes), self.args.num_display_samples)):
            if self.args.stroke_type == 'stroke-discrete':
                ...
                tmp_stroke_incomp = rec_incomplete_strokes(disc2stroke5(strokes[i].cpu().numpy(), self.args.max_size),
                                                           masks[i].cpu().numpy())
            else:
            ...
            imgs_incomp.append(strokes2drawing(tmp_stroke_incomp, size=128,
                                               svg_filename='{}/tmp_incomp_{}.svg'.format(self.tmp_dir, i)))
            # imgs_pred.append(strokes2drawing(tmp_stroke_pred,  size=128, svg_filename='{}/tmp_pred_{}.svg'.format(self.tmp_dir, i)))
            imgs_comp_pred.append(strokes2drawing(tmp_comp_stroke_pred, size=128,
                                                  svg_filename='{}/tmp_comp_pred_{}.svg'.format(self.tmp_dir, i)))
            record_img = np.concatenate([imgs[-1], imgs_incomp[-1], imgs_comp_pred[-1]],
                                        axis=1)  # , imgs_pred[-1], imgs_comp_pred[-1], imgs_incomp[-1],imgs_pred[-1], imgs_comp_pred[-1]
            self.tensorboard_logger.add_image('{}_record_imgs_{}'.format(mode, i),
                                              record_img.reshape((1,) + record_img.shape), self.counters['t'])

        # 保存
        if output:
            names = {}
            shot, prob = output
            cls = np.load('cls2name.npy', allow_pickle=True).item()
            for i in range(min(len(strokes), self.args.num_display_samples)):
                cls_name = cls[int(targets[i].cpu())]
                if cls_name not in names.keys():
                    names[cls_name] = 0
                    if not os.path.exists('Generate_oracle_fs/oracle_200_{}_shot/'.format(shot) + cls_name):
                        os.makedirs('Generate_oracle_fs/oracle_200_{}_shot/'.format(shot) + cls_name)
                else:
                    names[cls_name] += 1
                file_name = cls_name + '_' + str(names[cls_name]) + '_' + str(prob)
                plt.imsave('Generate_oracle_fs/oracle_200_{}_shot/{}/{}.jpg'.format(shot, cls_name,
                                                                                    file_name),
                           imgs_comp_pred[i], cmap='gray')
