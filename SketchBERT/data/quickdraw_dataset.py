...
class QuickDrawImageSBIRDataset(Dataset):
    ...
    def __init__(self, sketch_sum_path, image_sum_path, mode, image_size, get_type, max_cls_cache, each_max_samples,
                 each_image_max_samples):
        ...
        with open(self.image_sum_path, 'r') as rf:
            for line in rf:
                ...
                if cls_name in self.name2cls:
                    ...
                    if cls_index in self.image_cls2index and len(self.image_cls2index[cls_index]) >= (
                            int(self.mode == 'train') * self.each_image_max_samples + int(
                            self.mode == 'valid') * self.each_max_samples):
                        continue
...
class QuickDrawMemMapDataset(Dataset):
    ...
    def __init__(self, sum_path, offset_path, mode, task_types, get_type, max_length=250, max_size=[256, 256],
                 type_size=3, mask_prob=0.8, limit=1000,
                 stroke_type='stroke-5', input_is_complete=True, mask_task_type='task', normalization_type='var_scale',
                 max_scale_factor=10, max_cls_cache=500, each_max_samples=10000, cls_limit_path=None):
        self.mode = mode
    ...
    def get_term(self, index):
        ...
        sketches = np.memmap(path, dtype=np.int16, mode='r',
                             shape=(self.offsets[self.mode][self.cls2name[skt_ind[0]]][-1][-1], 3))
        sketch = sketches[skt_ind[1][0]:skt_ind[1][1], :].copy()
        del sketches
        cls = self.classes[index]
        sketch, segment, length_mask = self.transform_stroke(sketch, self.stroke_type, self.normalization_type,
                                                             self.max_length, self.max_size, self.type_size)
        # print(sketch)
    ...
    def get_mask_result(self, stroke, segment, length_mask, cls):
        ...
        if self.cls_in_input:
            ...
            length_mask = torch.cat([cls_input[0, 0:1].type(dtype=torch.float), length_mask.type(dtype=torch.float)],
                                    dim=0)
    ...
    def get_triplet(self, index):
        ...
        return (mask_input, pos_mask_input, neg_mask_input), (stroke, pos_stroke, neg_stroke), (
        segment, pos_segment, neg_segment), (length_mask, pos_length_mask, neg_length_mask), (
               mask, pos_mask, neg_mask), (cls, pos_cls, neg_cls)
    ...
    def load_dataset(self, sum_path, offset_path, cls_limit_path):
        ...
        if not os.path.exists('cls2name.npy'):
            np.save('cls2name.npy', self.cls2name)
...
class QuickDrawSBIRDataset(Dataset):
    ...
    def __init__(self, sketch_sum_path, sketch_offset_path, image_sum_path, mode, task_types, get_type, max_length=250,
                 max_size=[256, 256], image_size=224, type_size=3, mask_prob=0.8, limit=1000,
                 stroke_type='stroke-5', input_is_complete=True, mask_task_type='task', normalization_type='var_scale',
                 max_scale_factor=10, max_cls_cache=500, each_max_samples=10000, each_image_max_samples=10000,
                 cls_limit_path=None):
        self.mode = mode
        self.task_types = task_types
        self.get_type = get_type
