        ...
        for oracle in self.oracle_to_class.keys():
            ...
            for file in files:
                if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
                    ...
                    # img = Image.fromarray(cv2.threshold(np.array(img).reshape(img.size[0], img.size[1]),
                    #                                        0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1])
                    if self.type == 'test':
                        img = Image.open(os.path.join(path, file)).convert('RGB')
                        self.data.append([self.transform(img), self.oracle_to_class[oracle]])
                    if self.type == 'train':
                        self.data.append([basic_transfrom(img.convert('RGB')), self.oracle_to_class[oracle]])
                        if enlarge > 0 and self.transform is not None:
                            for _ in range(self.enlarge):
                                ...
                                tr_img = self.transform(img.convert('RGB'))  # \
